...
SwanIntgratSpc.$(EXTO) \
SwanBndStruc.$(EXTO) \
SwanVTKWriteHeader.$(EXTO) \
SwanVTKWriteData.$(EXTO) \
SwanVTKPDataSets.$(EXTO) \
ocpids.$(EXTO) \
ocpcre.$(EXTO) \
ocpmix.$(EXTO) \
fftpack51.$(EXTO)
...
help:
	...
	@echo "make merge     -- makes the    MPI $(SWANMERGE_EXE) executable"
...
merge:
	@perl switch.pl $(swch) -merge -mpi *.ftn *.ftn90
	$(MAKE) FOR=$(F90_MPI) FFLAGS="$(FLAGS_OPT) $(FLAGS_MSC) $(FLAGS_MPI)" \
	        FFLAGS90="$(FLAGS_OPT) $(FLAGS90_MSC) $(FLAGS_MPI)" \
                INCS="$(INCS_MPI)" LIBS="$(LIBS_MPI)" OBJS="$(SWAN_OBJS)" $(SWANMERGE_EXE)
...
$(SWANMERGE_EXE): $(SWAN_OBJS)
	$(FOR) $(OBJS) $(FFLAGS) $(OUT)$(SWANMERGE_EXE) $(INCS) $(LIBS)
...
clobber:
	$(RM) *.$(EXTO) *.mod *.f *.for *.f90 $(SWAN_EXE) $(SWANMERGE_EXE) $(HCAT_EXE) $(SWAN_LIB)
